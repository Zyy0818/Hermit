...
def _probe_no_tool_bypass() -> None:
    ...
    runtime = AgentRuntime(
        provider=SimpleNamespace(name="claim-probe", features=ProviderFeatures()),
        registry=ToolRegistry(),
...
def _probe_durable_reentry() -> None:
    ...
    with TemporaryDirectory() as tmpdir:
        try:
            base = Path(tmpdir)
            store, _artifacts, _controller, executor, ctx, workspace = _probe_task_runtime(
...
def _probe_memory_evidence() -> None:
    ...
    with TemporaryDirectory() as tmpdir:
        try:
            ...
            inspection = MemoryGovernanceService().inspect_claim(
                category="项目约定",
