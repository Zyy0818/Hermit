...
def test_memory_record_roundtrips_new_schema_fields(tmp_path: Path) -> None:
    ...
    try:
        ...
        loaded = store.get_memory_record(record.memory_id)
...
def test_active_memory_query_excludes_expired_invalidated_and_revoked(tmp_path: Path) -> None:
    store = KernelStore(tmp_path / "state.db")
    try:
        ...
        active_records = store.list_memory_records(status="active", limit=50)
