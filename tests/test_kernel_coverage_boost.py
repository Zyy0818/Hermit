...
class _ReceiptStore:
    ...
    def list_receipts(self, *, task_id: str, limit: int = 50):  # noqa: ARG002
...
def test_outcome_helpers_strip_runtime_markup_and_build_terminal_summary() -> None:
    ...
    receipts = [
        type("Receipt", (), {"output_refs": ["artifact-1", "artifact-1", "artifact-2", "", None]})(),
...
def test_planning_service_returns_none_without_selection_and_locates_latest_plan_attempt(tmp_path: Path) -> None:
    store = KernelStore(tmp_path / "kernel.db")
    artifacts = ArtifactStore(tmp_path / "artifacts")
    controller = TaskController(store)
    planning = PlanningService(store, artifacts)
...
def test_provider_input_compile_materializes_large_ingress_and_updates_attempt_context(tmp_path: Path) -> None:
    store = KernelStore(tmp_path / "kernel.db")
    artifacts = ArtifactStore(tmp_path / "artifacts")
    controller = TaskController(store)
    ...
    assert attempt.context["ingress_metadata"]["detected_payload_kinds"] == ["code_block", "long_text"]
    assert attempt.context["ingress_metadata"]["ingress_artifact_refs"] == compiled.ingress_artifact_refs
    ...
    assert all(store.get_artifact(ref).kind != "context.pack/v3" for ref in related_refs if store.get_artifact(ref))


def test_provider_input_compile_clears_input_dirty_and_emits_recompiled_event(tmp_path: Path) -> None:
    store = KernelStore(tmp_path / "kernel.db")
    artifacts = ArtifactStore(tmp_path / "artifacts")
    controller = TaskController(store)
    compiler = ProviderInputCompiler(store, artifacts)
...
def test_ingress_router_handles_explicit_reply_focus_and_branch_bindings(tmp_path: Path) -> None:
    ...
    secondary = _start_task(controller, conversation_id="conv-router", goal="查询北京天气", kind="respond")
...
def test_task_controller_helper_predicates_cover_followup_and_terminal_candidates(tmp_path: Path) -> None:
    ...
    other_ctx = _start_task(controller, conversation_id="conv-helper", goal="做数据迁移", kind="analyze")
    ...
    assert controller._ingress_queue_priority(source_channel="chat", requested_by=None, metadata={}) == 100
    assert controller._ingress_queue_priority(source_channel="scheduler", requested_by=None, metadata={}) == 10
    assert controller._ingress_queue_priority(
        source_channel="webhook", requested_by=None, metadata={"resume_kind": "approval"}
    ) == 90
    assert controller._ingress_queue_priority(source_channel="system", requested_by=None, metadata={}) == 0
    ...
    assert controller._looks_like_task_followup("刚才那个版本计划", task_id=respond_ctx.task_id) is True
    assert controller._sanitize_context_text(
    ...
    assert controller._texts_overlap("北京天气", "北京天气更新", query_tokens={"北京", "天气"}) is True
