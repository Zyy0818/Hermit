...
from hermit.builtin.web_tools import fetch, search
...
def test_fetch_helpers_cover_errors_and_text_extraction(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        ...
        "urlopen",
        lambda req, timeout=15: (_ for _ in ()).throw(urllib.error.HTTPError(req.full_url, 404, "Not Found", {}, None)),
    )
    ...
    """
    monkeypatch.setattr(fetch.urllib.request, "urlopen", lambda req, timeout=15: _FakeResponse(html))
    ...
    assert fetch._html_to_text("<div>hello<br>world</div><style>ignored</style>") == "hello\n\nworld"
...
def test_search_helpers_cover_results_parsing_and_errors(monkeypatch) -> None:
    ...
    monkeypatch.setattr(search, "_ddg_lite_search", lambda *args, **kwargs: "## Search Results\n### 1. Example")
    ...
    monkeypatch.setattr(search, "_ddg_lite_search", lambda *args, **kwargs: (_ for _ in ()).throw(RuntimeError("network")))
    ...
    assert search.handle_search({"query": "life", "search_type": "news"}) == "No results found for: life"
    ...
    monkeypatch.setattr(search.urllib.request, "urlopen", lambda req, timeout=10: _FakeResponse(b"<html></html>"))
    lite = search._ddg_lite_search("example", max_results=1, region="us-en", time_filter="day", search_type="news")
    assert "## Search Results" in lite
    assert "https://example.com" in lite
    assert "Snippet text" in lite

    assert search._extract_real_url("https://duckduckgo.com/l/?uddg=https%3A%2F%2Fexample.com") == "https://example.com"
    ...
    monkeypatch.setattr(
        ...
        "urlopen",
        lambda req, timeout=5: _FakeResponse(json.dumps(payload).encode("utf-8"), content_type="application/json"),
    )
...
def test_computer_use_run_command_and_osascript(monkeypatch) -> None:
    monkeypatch.setattr(
        actions.subprocess,
        "run",
        lambda args, capture_output=True, text=True: subprocess.CompletedProcess(args, 0, stdout="ok", stderr=""),
    )
    ...
    monkeypatch.setattr(
        actions.subprocess,
        "run",
        lambda args, capture_output=True, text=True: subprocess.CompletedProcess(args, 1, stdout="", stderr="bad"),
    )
    with pytest.raises(RuntimeError, match="bad"):
        actions._run_command(["echo", "bad"])

    captured: list[list[str]] = []
    monkeypatch.setattr(actions, "_run_command", lambda args: captured.append(args) or subprocess.CompletedProcess(args, 0, stdout="", stderr=""))
...
def test_screenshot_and_pointer_actions(monkeypatch, tmp_path) -> None:
    ...
    monkeypatch.setattr(actions, "_run_command", lambda args: clicks.append(args) or subprocess.CompletedProcess(args, 0, stdout="", stderr=""))
    assert actions.click({"x": 1, "y": 2, "button": "right"}) == {"ok": True, "info": "clicked at 1,2"}
    ...
    monkeypatch.setattr(actions, "_run_osascript", lambda script, argv=None: scripts.append((script, argv)) or subprocess.CompletedProcess([], 0, stdout="", stderr=""))
    assert actions.click({"x": 3, "y": 4}) == {"ok": True, "info": "click at 3,4"}
    assert actions.move({"x": 5, "y": 6}) == {"ok": True, "info": "moved to 5,6"}
    assert actions.scroll({"x": 7, "y": 8, "direction": "down", "amount": 2}) == {"ok": True, "info": "scrolled down at 7,8"}
...
def test_typing_keys_screen_size_and_open_app(monkeypatch) -> None:
    ...
    monkeypatch.setattr(actions, "_run_command", lambda args: calls.append(args) or subprocess.CompletedProcess(args, 0, stdout="", stderr=""))
    ...
    assert actions.scroll({"x": 1, "y": 2, "direction": "up", "amount": 3}) == {"ok": True, "info": "scrolled up at 1,2"}
    ...
    monkeypatch.setattr(actions, "_tool_exists", lambda name: False)
    scripts: list[tuple[str, list[str] | None]] = []
    monkeypatch.setattr(actions, "_run_osascript", lambda script, argv=None: scripts.append((script, argv)) or subprocess.CompletedProcess([], 0, stdout="0, 0, 1440, 900", stderr=""))
    ...
    monkeypatch.setattr(actions, "_run_osascript", lambda script, argv=None: subprocess.CompletedProcess([], 0, stdout="bad", stderr=""))
