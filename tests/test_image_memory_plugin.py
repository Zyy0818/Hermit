...
from hermit.builtin.image_memory.hooks import _parse_json, register
...
def test_image_store_from_path_persists_asset_and_indexes(tmp_path: Path, monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        "hermit.builtin.image_memory.hooks._analyze_image",
...
def test_image_attach_to_feishu_returns_custom_tag(tmp_path: Path, monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        "hermit.builtin.image_memory.hooks._build_lark_client",
        lambda: object(),
    )
    monkeypatch.setattr(
        "hermit.builtin.image_memory.hooks._upload_image_to_feishu",
