...
from hermit.builtin.scheduler import tools as scheduler_tools
from hermit.builtin.scheduler.models import JobExecutionRecord, ScheduledJob
from hermit.builtin.webhook import tools as webhook_tools
...
def test_scheduler_tools_cover_create_list_delete_history_update_and_register(monkeypatch) -> None:
    ...
    assert scheduler_tools._handle_create({}) == "Error: name, prompt, and schedule_type are required."
    assert scheduler_tools._handle_create({"name": "job", "prompt": "run", "schedule_type": "weird"}) == (
        "Error: schedule_type must be 'cron', 'once', or 'interval'."
    )
    assert scheduler_tools._handle_create({"name": "job", "prompt": "run", "schedule_type": "cron"}) == (
    ...
    assert scheduler_tools._handle_create({"name": "job", "prompt": "run", "schedule_type": "once"}) == (
    ...
    assert scheduler_tools._handle_create(
        {"name": "job", "prompt": "run", "schedule_type": "once", "once_at": "1970-01-01T00:01:00"}
    ...
    assert scheduler_tools._handle_delete({"job_id": "missing"}) == "Error: no task with id 'missing' found."
    assert scheduler_tools._handle_delete({"job_id": listed_job.id}) == f"Deleted scheduled task '{listed_job.id}'."
    ...
    assert "invalid cron expression" in scheduler_tools._handle_update({"job_id": "job-2", "cron_expr": "bad cron"})
    ...
    assert scheduler_tools._handle_update({"job_id": "missing", "name": "x"}) == "Error: no task with id 'missing' found."
...
def test_webhook_tools_cover_file_crud_and_register(tmp_path: Path) -> None:
    ...
    assert webhook_tools._handle_delete({"name": "missing"}) == "Error: route 'missing' not found. Existing routes: github"

    assert webhook_tools._handle_update({}) == "Error: name is required."
    assert webhook_tools._handle_update({"name": "missing"}) == "Error: route 'missing' not found. Existing routes: github"
    ...
    assert "Webhook route 'github' updated: prompt_template, path, secret, feishu_chat_id." in updated_text
