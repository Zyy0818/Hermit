...
def _load_build_macos_dmg_module():
    module_path = Path(__file__).resolve().parents[1] / "scripts" / "build_macos_dmg.py"
...
def test_build_dmg_creates_stage_bundle_and_applications_link(
    ...
    monkeypatch.setattr(build_macos_dmg, "app_name", lambda base_dir=None: "Hermit Menu Dev")
