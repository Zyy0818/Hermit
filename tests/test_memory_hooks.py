...
def test_inject_memory_returns_empty_when_no_entries(tmp_path) -> None:
    ...
    with patch.object(engine, "summary_prompt", return_value=""), patch.object(hooks.log, "info") as log_mock:
...
def test_registered_hooks_fire_with_context(tmp_path) -> None:
    ...
    engine.save({"项目约定": [MemoryEntry(category="项目约定", content="默认工作目录固定到 /repo", score=8, locked=True)]})
    ...
    assert "<relevant_memory>" in pre_run
...
def test_save_memories_returns_early_without_messages(tmp_path) -> None:
    ...
    with patch.object(hooks, "_extract_and_save") as extract_mock, patch.object(hooks.log, "info") as log_mock:
...
def test_save_memories_logs_exception_and_clears_progress(tmp_path) -> None:
    ...
    with patch.object(hooks, "_extract_and_save", side_effect=RuntimeError("boom")), patch.object(
        hooks.log, "exception"
    ) as log_mock, patch.object(hooks, "_clear_session_progress") as clear_mock:
        hooks._save_memories(engine, settings, "s1", [{"role": "user", "content": "hello"}])
...
def test_checkpoint_memories_returns_early_for_skipped_conditions(tmp_path) -> None:
    ...
    with patch.object(hooks, "_pending_messages") as pending_mock, patch.object(hooks.log, "info") as log_mock:
        hooks._checkpoint_memories(engine, settings, "", messages)
        hooks._checkpoint_memories(engine, settings, "cli-oneshot", messages)
        hooks._checkpoint_memories(engine, _settings(tmp_path / "other", has_auth=False), "s1", messages)
...
def test_checkpoint_memories_returns_when_no_pending_delta(tmp_path) -> None:
    ...
    with patch.object(hooks, "_pending_messages", return_value=([], 3)), patch.object(
        hooks, "_extract_memory_payload"
    ) as extract_mock, patch.object(hooks.log, "info") as log_mock:
        hooks._checkpoint_memories(engine, settings, "s1", [{"role": "user", "content": "记住这个约定"}])
...
def test_checkpoint_memories_returns_when_below_threshold(tmp_path) -> None:
    ...
    with patch.object(hooks, "_pending_messages", return_value=(delta, 0)), patch.object(
        hooks, "_should_checkpoint", return_value=(False, "below_threshold")
    ), patch.object(hooks, "_extract_memory_payload") as extract_mock, patch.object(hooks.log, "info") as log_mock:
...
def test_checkpoint_memories_logs_exception_on_extract_failure(tmp_path) -> None:
    ...
    ), patch.object(hooks, "_extract_memory_payload", side_effect=RuntimeError("boom")), patch.object(
        hooks.log, "exception"
    ) as log_mock:
        hooks._checkpoint_memories(engine, settings, "s1", delta)
...
def test_checkpoint_memories_logs_when_no_new_entries(tmp_path) -> None:
    settings = _settings(tmp_path)
    engine = MemoryEngine(settings.memory_file)
    messages = [{"role": "user", "content": "记住这个约定"}, {"role": "assistant", "content": "好的"}]

    with patch.object(hooks, "_pending_messages", return_value=(messages, 1)), patch.object(
        hooks, "_should_checkpoint", return_value=(True, "explicit_memory_signal")
    ), patch.object(
        hooks, "_extract_memory_payload", return_value={"used_keywords": set(), "new_entries": []}
    ), patch.object(
        hooks.log, "info"
...
def test_checkpoint_memories_appends_and_marks_processed(tmp_path) -> None:
    settings = _settings(tmp_path)
    engine = MemoryEngine(settings.memory_file)
    messages = [{"role": "user", "content": "记住：服务端口改为 8080"}, {"role": "assistant", "content": "收到"}]
...
def test_extract_and_save_returns_when_nothing_extracted(tmp_path) -> None:
    ...
    with patch.object(
        hooks, "_extract_memory_payload", return_value={"used_keywords": set(), "new_entries": []}
    ), patch.object(hooks.log, "info") as log_mock, patch.object(engine, "record_session") as record_mock:
...
def test_extract_memory_payload_returns_empty_for_short_transcript(tmp_path) -> None:
    ...
    with patch.object(hooks.log, "info") as log_mock:
        payload = hooks._extract_memory_payload(engine, settings, [{"role": "user", "content": "x"}], max_tokens=100)
...
def test_extract_memory_payload_returns_empty_when_service_returns_none(tmp_path) -> None:
    ...
    with patch.object(hooks, "build_provider", return_value=object()) as provider_mock, patch.object(
        hooks, "StructuredExtractionService", return_value=service
    ) as service_cls, patch.object(hooks.log, "info") as log_mock:
        payload = hooks._extract_memory_payload(
            engine,
            settings,
            [{"role": "user", "content": "this transcript is definitely long enough to extract memory"}],
            max_tokens=100,
        )
...
def test_extract_memory_payload_builds_entries_and_skips_empty_content(tmp_path) -> None:
    ...
    with patch.object(hooks, "build_provider", return_value=object()), patch.object(
        hooks, "StructuredExtractionService", return_value=service
...
def test_consolidate_category_entries_merges_supersede_history() -> None:
    newer = MemoryEntry(category="项目约定", content="默认工作目录固定到 /repo", supersedes=["旧约定 A"])
    older = MemoryEntry(category="项目约定", content="默认工作目录使用 /repo", supersedes=["旧约定 B"])
...
def test_clear_session_progress_removes_existing_session(tmp_path) -> None:
    state_file = tmp_path / "session_state.json"
    state_file.write_text('{"session_index":2,"sessions":{"s1":{"processed_messages":4},"s2":{"processed_messages":1}}}', encoding="utf-8")
...
def test_message_text_handles_non_string_and_skips_invalid_blocks() -> None:
    ...
    assert hooks._message_text({"role": "assistant", "content": [None, {"type": "text", "text": "ok"}]}) == "ok"
...
def test_should_checkpoint_conversation_batch() -> None:
    long_text = "这里是一段很长的背景材料，用于测试批量对话长度阈值，内容仅描述一般情况和上下文信息。" * 6
    should_checkpoint, reason = hooks._should_checkpoint([
...
def test_bump_session_index_returns_fallback_on_update_error(tmp_path) -> None:
    ...
    with patch.object(hooks.JsonStore, "update", side_effect=RuntimeError("boom")), patch.object(
        hooks.log, "warning"
    ) as warning_mock:
