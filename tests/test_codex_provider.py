...
def test_responses_input_maps_tool_history_and_images() -> None:
    ...
    assert items[0]["content"][1] == {"type": "input_image", "image_url": "https://example.com/a.png"}
    assert items[1] == {
        "type": "function_call",
        "call_id": "call_1",
        "name": "read_file",
...
def test_codex_provider_generate_parses_text_and_tool_calls(monkeypatch, tmp_path: Path) -> None:
    ...
    assert captured["body"]["input"] == [{"type": "message", "role": "user", "content": [{"type": "input_text", "text": "hello"}]}]
...
def test_codex_oauth_provider_generate_parses_sse_blocks(monkeypatch, tmp_path: Path) -> None:
    ...
    assert captured["body"]["instructions"] == "system rules"
    assert response.stop_reason == "tool_use"
    assert response.content == [{"type": "tool_use", "id": "call_1", "name": "bash", "input": {"command": "pwd"}}]
