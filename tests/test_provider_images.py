...
from hermit.provider import images as provider_images
...
def test_compress_image_bytes_handles_short_circuit_success_and_failure(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        ...
        lambda image_bytes, media_type, max_bytes: (_ for _ in ()).throw(ImportError("missing pillow")),
    )
    monkeypatch.setattr(
        provider_images,
        "_compress_with_sips",
    ...
    monkeypatch.setattr(
        provider_images,
        "_compress_with_sips",
        lambda image_bytes, media_type, max_bytes: (_ for _ in ()).throw(RuntimeError("sips failed")),
    )
...
def test_encode_candidates_and_downscale_image(monkeypatch) -> None:
    ...
    monkeypatch.setattr(provider_images, "_save_webp", lambda image, quality: f"webp-{quality}".encode("ascii"))
    monkeypatch.setattr(provider_images, "_save_jpeg", lambda image, quality: f"jpeg-{quality}".encode("ascii"))
    ...
    downscaled = provider_images._downscale_image(FakeImage(100, 50), current_size=10_000, max_bytes=100, resampling="LANCZOS")
...
def test_save_helpers_and_compress_with_pillow(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        ...
        lambda image, media_type: [("image/png", b"x" * 200)] if image.width > 1 else [("image/jpeg", b"x" * 50)],
    )
    monkeypatch.setattr(
        provider_images,
        "_downscale_image",
        lambda image, current_size, max_bytes, *, resampling: image.resize((1, 1), resample=resampling),
    )

    media_type, compressed = provider_images._compress_with_pillow(source.getvalue(), "image/png", 100)
...
def test_sips_helpers_parse_dimensions_and_compress(monkeypatch) -> None:
    ...
    def fake_run(cmd: list[str], *, check: bool, capture_output: bool, text: bool):
        calls.append(cmd)
        if "-g" in cmd:
            return subprocess.CompletedProcess(cmd, 0, stdout="pixelWidth: 300\npixelHeight: 200\n", stderr="")
    ...
    assert provider_images._compress_with_sips(b"raw-image-bytes", "image/png", 10) == ("image/jpeg", b"tiny")
