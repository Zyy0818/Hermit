...
if [[ -z "${ENV_NAME}" ]]; then
  echo "Usage: scripts/hermit-menubar-env.sh <prod|dev|test> [--adapter <adapter>] [--profile <profile>]" >&2
  exit 1
fi
...
exec /opt/homebrew/bin/uv run --project "${ROOT_DIR}" --python 3.11 python -m hermit.companion.menubar "$@"
